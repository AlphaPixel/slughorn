...
public:
    ...
    //               Pass a fresh namespace per Canvas (e.g. 0xE0000) or share one across
    //               multiple Canvas instances that write to the same Atlas.
    ...
    // Arc — circular arc centred at (cx,cy) with radius r, from startAngle to
    // endAngle (radians, measured from the +X axis, Y-up convention).
    //
    // @p ccw — if true, sweep counter-clockwise; clockwise otherwise.
    //          Matches HTML Canvas arc() exactly.
    //
    // The arc is decomposed into segments of at most π/2 (quarter-circle) each.
    // Each segment is emitted as a cubic via bezierTo(), which routes through the
    // adaptive CurveDecomposer — so quality scales automatically with curvature.
    //
    // arc() does NOT call beginPath(). It appends to the current path, so you can
    // combine arcs with lineTo() to build pie slices, stadium shapes, etc. If you
    // want a standalone arc shape, call beginPath() first.
    //
    ...
    void arc(slug_t cx, slug_t cy, slug_t r, slug_t startAngle, slug_t endAngle, bool ccw = false) {
        // Normalise the sweep so it is always positive and ≤ 2π.
        // HTML Canvas semantics: if ccw, sweep is endAngle→startAngle going CCW.
        slug_t sweep;

        if(ccw) {
            // Swap so we always march from lower to higher angle, then negate
            // to signal direction to _arcSegments.
    ...
    void arcTo(slug_t x1, slug_t y1, slug_t x2, slug_t y2, slug_t r) {
        ...
        // Distance from corner to the two tangent points (= r / tan(halfAngle)).
        // dot = cos of angle between legs; half-angle formula:
        //   tan(halfAngle) = sin(halfAngle)/cos(halfAngle)
        //                  = |cross| / (1 + dot)   via half-angle identity
    ...
    // @p scale  — normalizes authoring-space coords into em-space (default 1.0).
    //             NEVER stored in Layer::scale; see Scale Contract in the file header.
    //
    // The curves are shifted to local origin (tight atlas bands); the canvas-space
    // offset is stored in Layer::transform (dx/dy). An internal key is
    // auto-generated and _baseKey is incremented.
    //
    ...
    // Register the in-progress CompositeShape in the Atlas under @p key and
    // reset internal state. Equivalent to:
    //
...
private:
    ...
    //
    // Splits the sweep into segments of at most π/2 each (quarter-circle), which
    // keeps the cubic approximation error below 0.00027% of r. Each segment is a
    // single bezierTo() → CurveDecomposer::cubicTo() → adaptive subdivision.
    //
    // Does NOT call moveTo(). The caller (arc() / arcTo()) is responsible for
    // ensuring the current point is at the arc start before calling this.
    void _arcSegments(slug_t cx, slug_t cy, slug_t r, slug_t startAngle, slug_t sweep) {
        if(r <= 0.0_cv || sweep == 0.0_cv) return;

        // Number of segments: ceil(|sweep| / (π/2)), minimum 1.
        ...
        for(int i = 0; i < nSegs; ++i) {
            ...
            // The tangent at angle a is (-sin(a), cos(a)) for CCW; flip for CW.
            const slug_t p1x = p0x - k * r * sin0;
            const slug_t p1y = p0y + k * r * cos0;
            const slug_t p2x = p3x + k * r * sin1;
            const slug_t p2y = p3y - k * r * cos1;

            // On the very first segment, snap the start to a moveTo if the path
            // is empty; otherwise emit a lineTo to connect cleanly.
            if(i == 0 && _pendingCurves.empty()) {
                moveTo(p0x, p0y);
            } else if(i == 0) {
                // Connect current position to arc start with a line.
                // (arc() callers that want a seamless join should ensure their
                //  current point is already at the arc start.)
    ...
    // Apply scale to every coordinate in a curve list.
    // Fast-path: returns src unchanged (no copy) when scale == 1.
    ...
    // Shift curves to local origin (bounding-box minimum → 0,0).
    // The subtracted offset is written to @p outTransform (dx/dy only).
    // Returns an empty vector if the bounding box is degenerate.
