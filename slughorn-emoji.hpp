...
// =============================================================================
// slughorn-emoji.hpp — emoji name / codepoint lookup table for slughorn
//
// Provides a bidirectional map between CLDR short names (e.g. "dragon",
// "puzzle_piece") and Unicode codepoints (e.g. 0x1F409, 0x1F9E9).
//
// No dependencies beyond slughorn.hpp and the C++ standard library.
// No graphics library, no FreeType, no Skia, no Cairo.
//
// USAGE
// -----
...
// ZWJ SEQUENCES
// -------------
// Multi-codepoint sequences (e.g. 👨‍👩‍👧 family emoji) are not stored in this
// table — they require a different key encoding.  nameToCodepoint() returns
// std::nullopt for any name that would map to a sequence.
// This is the natural forcing function for the Atlas::Key refactor (see the
// TODO in slughorn.hpp).
//
// DATA SOURCE
// -----------
// Unicode 15.1 RGI emoji set, CLDR short names.
// Regenerate with tools/gen_emoji_table.py against a future Unicode release.
// =============================================================================
...
// Return the Unicode codepoint for a CLDR short name, or std::nullopt if the
// name is not in the table.
//
// @p name must be pre-normalised (lowercase, underscores).  Slack-style colons
...
// =============================================================================
// Static table — sorted by name for binary search
//
// Generated from Unicode 15.1 CLDR short names.
// Do not edit by hand — regenerate with tools/gen_emoji_table.py.
// =============================================================================
...
// codepointToName
//
// We keep a separate index array sorted by codepoint so that the primary table
// stays sorted by name (needed for the binary search above).
// The index is built once on first use via a local static.
// =============================================================================
...
std::optional<std::string_view> codepointToName(uint32_t codepoint) {
    ...
    struct ByCodepoint {
        ...
        bool operator<(const ByCodepoint& o) const {
    ...
    static const auto kByCodepoint = []() {
        ...
        for(size_t i = 0; i < kTableSize; i++) {
            t.entries[i] = {kTable[i].codepoint, kTable[i].name};
        }
...
#endif // SLUGHORN_EMOJI_IMPLEMENTATION
