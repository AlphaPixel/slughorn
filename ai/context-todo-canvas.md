...
arc() decomposes into segments of at most π/2 each via _arcSegments(), using:
...
Practical curve count with TOLERANCE_BALANCED (confirmed in testing):
    triangle  →   6 curves  (3 edges × 2 quads, no cubics involved)
    circle    → ~32 curves  (4 cubics × ~8 quads at BALANCED)
...
- Rect → expect 8 curves (4 edges × 2 quads each)
- Circle via circle() at TOLERANCE_EXACT → expect 8 curves (4 cubics × 2 quads)
- Circle via circle() at TOLERANCE_BALANCED → expect ~32 curves

### arc() / arcTo()
- arc() full circle (0 → 2π) → same curve count as circle()
- arc() semicircle (0 → π) → roughly half the curves of a full circle
...
- MAX_DEPTH clamp: degenerate cubic → at most 2^8 × 2 = 512 quads
